...
control MyIngress(inout headers hdr,
    ...
    * Number of operations:
    ...
    action send_broadcast(in bit<32> index) {
        ...
        max_pool_registry.read(hdr.p4calc.res[7:0]    ,index);
        max_pool_registry.read(hdr.p4calc.res[15:8]   ,index + 1);
        max_pool_registry.read(hdr.p4calc.res[23:16]  ,index + 2);
        max_pool_registry.read(hdr.p4calc.res[31:24]  ,index + 3);
        max_pool_registry.read(hdr.p4calc.res[39:32]  ,index + 4);
        max_pool_registry.read(hdr.p4calc.res[47:40]  ,index + 5);
        max_pool_registry.read(hdr.p4calc.res[55:48]  ,index + 6);
        max_pool_registry.read(hdr.p4calc.res[63:56]  ,index + 7);
    ...
    action conv_maxpool_calc(in bit<40> f, in bit<32> index ) {
        bit<8> conv =
        hdr.p4calc.data[7:0]   * f[7:0]   +
        hdr.p4calc.data[15:8]  * f[15:8]  +
        hdr.p4calc.data[23:16] * f[23:16] +
        hdr.p4calc.data[31:24] * f[31:24] +
        hdr.p4calc.data[39:32] * f[39:32];
    ...
    action operation_calc() {
        ...
        bit<32> index = hdr.p4calc.max_pool_index * 8;
