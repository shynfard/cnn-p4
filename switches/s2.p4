...
header p4calc_t {
    ...
    bit<32>     switch1_max_pool_index;
    bit<32>     switch1_replication;
    bit<64>     input_data;
...
control MyIngress(inout headers hdr,
    ...
    * Number of operations:
    ...
    action send_broadcast(in bit<32> index) {
        ...
        max_pool_registry.read(hdr.p4calc.res[7:0],     index_read);
        max_pool_registry.read(hdr.p4calc.res[15:8],    index_read + 1);
        max_pool_registry.read(hdr.p4calc.res[23:16],   index_read + 2);
        max_pool_registry.read(hdr.p4calc.res[31:24],   index_read + 3);
        max_pool_registry.read(hdr.p4calc.res[39:32],   index_read + 4);
        max_pool_registry.read(hdr.p4calc.res[47:40],   index_read + 5);
        max_pool_registry.read(hdr.p4calc.res[55:48],   index_read + 6);
        max_pool_registry.read(hdr.p4calc.res[63:56],   index_read + 7);
        max_pool_registry.read(hdr.p4calc.res[71:64],   index_read + 8);
        max_pool_registry.read(hdr.p4calc.res[79:72],   index_read + 9);
        max_pool_registry.read(hdr.p4calc.res[87:80],   index_read + 10);
        max_pool_registry.read(hdr.p4calc.res[95:88],   index_read + 11);
        max_pool_registry.read(hdr.p4calc.res[103:96],  index_read + 12);
        max_pool_registry.read(hdr.p4calc.res[111:104], index_read + 13);
        max_pool_registry.read(hdr.p4calc.res[119:112], index_read + 14);
        max_pool_registry.read(hdr.p4calc.res[127:120], index_read + 15);
        hdr.p4calc.replication = replication;
        hdr.p4calc.index = index;
    ...
    action operation_insert() {
        bit<32> index = hdr.p4calc.switch1_max_pool_index * 32 + hdr.p4calc.switch1_replication * 8;
        input_data.write(index, hdr.p4calc.input_data[7:0]);
        input_data.write(index + 1, hdr.p4calc.input_data[15:8]);
        input_data.write(index + 2, hdr.p4calc.input_data[23:16]);
        input_data.write(index + 3, hdr.p4calc.input_data[31:24]);
        input_data.write(index + 4, hdr.p4calc.input_data[39:32]);
        input_data.write(index + 5, hdr.p4calc.input_data[47:40]);
        input_data.write(index + 6, hdr.p4calc.input_data[55:48]);
        input_data.write(index + 7, hdr.p4calc.input_data[63:56]);
    }
     ...
     * 
     */
    action operation_insertAndSend() {
        operation_insert();
        send_back();
    }
    ...
    table calculate {
        key = {
            hdr.p4calc.switch1_replication          : exact;
            hdr.p4calc.switch1_max_pool_index       : exact;
        }
        actions = {
            operation_insertAndSend;
            operation_drop;
        ...
        const entries = {
            (0x00, 0x0): operation_insertAndSend();
            (0x01, 0x0): operation_insertAndSend();
            (0x02, 0x0): operation_insertAndSend();
            (0x03, 0x0): operation_insertAndSend();
            (0x00, 0x1): operation_insertAndSend();
            (0x01, 0x1): operation_insertAndSend();
            (0x02, 0x1): operation_insertAndSend();
            (0x03, 0x1): operation_insertAndSend();
            (0x00, 0x2): operation_insertAndSend();
            (0x01, 0x2): operation_insertAndSend();
            (0x02, 0x2): operation_insertAndSend();
            (0x03, 0x2): operation_insertAndSend();
            (0x00, 0x3): operation_insertAndSend();
            (0x01, 0x3): operation_insertAndSend();
            (0x02, 0x3): operation_insertAndSend();
            (0x03, 0x3): operation_insertAndSend();
            (0x00, 0x4): operation_insertAndSend();
            (0x01, 0x4): operation_insertAndSend();
            (0x02, 0x4): operation_insertAndSend();
            (0x03, 0x4): operation_insertAndCalc(0, 0);
            (0x00, 0x5): operation_insertAndSend();
            (0x01, 0x5): operation_insertAndSend();
            (0x02, 0x5): operation_insertAndSend();
            (0x03, 0x5): operation_insertAndCalc(1, 0);
            (0x00, 0x6): operation_insertAndSend();
            (0x01, 0x6): operation_insertAndSend();
            (0x02, 0x6): operation_insertAndSend();
            (0x03, 0x6): operation_insertAndCalc(2, 0);
            (0x00, 0x7): operation_insertAndSend();
            (0x01, 0x7): operation_insertAndSend();
            (0x02, 0x7): operation_insertAndSend();
            (0x03, 0x7): operation_insertAndCalc(3, 0);
            (0x00, 0x8): operation_insertAndSend();
            (0x01, 0x8): operation_insertAndSend();
            (0x02, 0x8): operation_insertAndSend();
            (0x03, 0x8): operation_insertAndCalc(4, 0);
            (0x00, 0x9): operation_insertAndSend();
            (0x01, 0x9): operation_insertAndSend();
            (0x02, 0x9): operation_insertAndSend();
            (0x03, 0x9): operation_insertAndCalcAndSend(5, 1, 0);
            (0x00, 0xa): operation_insertAndSend();
            (0x01, 0xa): operation_insertAndSend();
            (0x02, 0xa): operation_insertAndSend();
            (0x03, 0xa): operation_insertAndCalc(6, 1);
            (0x00, 0xb): operation_insertAndSend();
            (0x01, 0xb): operation_insertAndSend();
            (0x02, 0xb): operation_insertAndSend();
            (0x03, 0xb): operation_insertAndCalc(7, 1);
            (0x00, 0xc): operation_insertAndSend();
            (0x01, 0xc): operation_insertAndSend();
            (0x02, 0xc): operation_insertAndSend();
            (0x03, 0xc): operation_insertAndCalc(8, 1);
            (0x00, 0xd): operation_insertAndSend();
            (0x01, 0xd): operation_insertAndSend();
            (0x02, 0xd): operation_insertAndSend();
            (0x03, 0xd): operation_insertAndCalc(9, 1);
            (0x00, 0xe): operation_insertAndSend();
            (0x01, 0xe): operation_insertAndSend();
            (0x02, 0xe): operation_insertAndSend();
            (0x03, 0xe): operation_insertAndCalcAndSend(10, 2, 1);
            (0x00, 0xf): operation_insertAndSend();
            (0x01, 0xf): operation_insertAndSend();
            (0x02, 0xf): operation_insertAndSend();
            (0x03, 0xf): operation_insertAndCalc(11, 2);
            (0x00, 0x10): operation_insertAndSend();
            (0x01, 0x10): operation_insertAndSend();
            (0x02, 0x10): operation_insertAndSend();
            (0x03, 0x10): operation_insertAndCalc(12, 2);
            (0x00, 0x11): operation_insertAndSend();
            (0x01, 0x11): operation_insertAndSend();
            (0x02, 0x11): operation_insertAndSend();
            (0x03, 0x11): operation_insertAndCalc(13, 2);
            (0x00, 0x12): operation_insertAndSend();
            (0x01, 0x12): operation_insertAndSend();
            (0x02, 0x12): operation_insertAndSend();
            (0x03, 0x12): operation_insertAndCalc(14, 2);
            (0x00, 0x13): operation_insertAndSend();
            (0x01, 0x13): operation_insertAndSend();
            (0x02, 0x13): operation_insertAndSend();
            (0x03, 0x13): operation_insertAndCalcAndSend(15, 3, 2);
            (0x00, 0x14): operation_insertAndSend();
            (0x01, 0x14): operation_insertAndSend();
            (0x02, 0x14): operation_insertAndSend();
            (0x03, 0x14): operation_insertAndCalc(16, 3);
            (0x00, 0x15): operation_insertAndSend();
            (0x01, 0x15): operation_insertAndSend();
            (0x02, 0x15): operation_insertAndSend();
            (0x03, 0x15): operation_insertAndCalc(17, 3);
            (0x00, 0x16): operation_insertAndSend();
            (0x01, 0x16): operation_insertAndSend();
            (0x02, 0x16): operation_insertAndSend();
            (0x03, 0x16): operation_insertAndCalc(18, 3);
            (0x00, 0x17): operation_insertAndSend();
            (0x01, 0x17): operation_insertAndSend();
            (0x02, 0x17): operation_insertAndSend();
            (0x03, 0x17): operation_insertAndCalc(19, 3);
            (0x00, 0x18): operation_insertAndSend();
            (0x01, 0x18): operation_insertAndSend();
            (0x02, 0x18): operation_insertAndSend();
            (0x03, 0x18): operation_insertAndCalcAndSend(20, 4, 3);
            (0x00, 0x19): operation_insertAndSend();
            (0x01, 0x19): operation_insertAndSend();
            (0x02, 0x19): operation_insertAndSend();
            (0x03, 0x19): operation_insertAndCalc(21, 4);
            (0x00, 0x1a): operation_insertAndSend();
            (0x01, 0x1a): operation_insertAndSend();
            (0x02, 0x1a): operation_insertAndSend();
            (0x03, 0x1a): operation_insertAndCalc(22, 4);
            (0x00, 0x1b): operation_insertAndSend();
            (0x01, 0x1b): operation_insertAndSend();
            (0x02, 0x1b): operation_insertAndSend();
            (0x03, 0x1b): operation_insertAndCalc(23, 4);
            (0x00, 0x1c): operation_insertAndSend();
            (0x01, 0x1c): operation_insertAndSend();
            (0x02, 0x1c): operation_insertAndSend();
            (0x03, 0x1c): operation_insertAndCalc(24, 4);
            (0x00, 0x1d): operation_insertAndSend();
            (0x01, 0x1d): operation_insertAndSend();
            (0x02, 0x1d): operation_insertAndSend();
            (0x03, 0x1d): operation_insertAndCalcAndSend(25, 5, 4);
            (0x00, 0x1e): operation_insertAndSend();
            (0x01, 0x1e): operation_insertAndSend();
            (0x02, 0x1e): operation_insertAndSend();
            (0x03, 0x1e): operation_insertAndCalc(26, 5);
            (0x00, 0x1f): operation_insertAndSend();
            (0x01, 0x1f): operation_insertAndSend();
            (0x02, 0x1f): operation_insertAndSend();
            (0x03, 0x1f): operation_insertAndCalc(27, 5);
            (0x00, 0x20): operation_insertAndSend();
            (0x01, 0x20): operation_insertAndSend();
            (0x02, 0x20): operation_insertAndSend();
            (0x03, 0x20): operation_insertAndCalc(28, 5);
            (0x00, 0x21): operation_insertAndSend();
            (0x01, 0x21): operation_insertAndSend();
            (0x02, 0x21): operation_insertAndSend();
            (0x03, 0x21): operation_insertAndCalc(29, 5);
            (0x00, 0x22): operation_insertAndSend();
            (0x01, 0x22): operation_insertAndSend();
            (0x02, 0x22): operation_insertAndSend();
            (0x03, 0x22): operation_insertAndCalcAndSend(30, 6, 5);
            (0x00, 0x23): operation_insertAndSend();
            (0x01, 0x23): operation_insertAndSend();
            (0x02, 0x23): operation_insertAndSend();
            (0x03, 0x23): operation_insertAndCalc(31, 6);
            (0x00, 0x24): operation_insertAndSend();
            (0x01, 0x24): operation_insertAndSend();
            (0x02, 0x24): operation_insertAndSend();
            (0x03, 0x24): operation_insertAndCalc(32, 6);
            (0x00, 0x25): operation_insertAndSend();
            (0x01, 0x25): operation_insertAndSend();
            (0x02, 0x25): operation_insertAndSend();
            (0x03, 0x25): operation_insertAndCalc(33, 6);
            (0x00, 0x26): operation_insertAndSend();
            (0x01, 0x26): operation_insertAndSend();
            (0x02, 0x26): operation_insertAndSend();
            (0x03, 0x26): operation_insertAndCalc(34, 6);
            (0x00, 0x27): operation_insertAndSend();
            (0x01, 0x27): operation_insertAndSend();
            (0x02, 0x27): operation_insertAndSend();
            (0x03, 0x27): operation_insertAndCalcAndSend(35, 7, 6);
            (0x00, 0x28): operation_insertAndSend();
            (0x01, 0x28): operation_insertAndSend();
            (0x02, 0x28): operation_insertAndSend();
            (0x03, 0x28): operation_insertAndCalc(36, 7);
            (0x00, 0x29): operation_insertAndSend();
            (0x01, 0x29): operation_insertAndSend();
            (0x02, 0x29): operation_insertAndSend();
            (0x03, 0x29): operation_insertAndCalc(37, 7);
            (0x00, 0x2a): operation_insertAndSend();
            (0x01, 0x2a): operation_insertAndSend();
            (0x02, 0x2a): operation_insertAndSend();
            (0x03, 0x2a): operation_insertAndCalc(38, 7);
            (0x00, 0x2b): operation_insertAndSend();
            (0x01, 0x2b): operation_insertAndSend();
            (0x02, 0x2b): operation_insertAndSend();
            (0x03, 0x2b): operation_insertAndCalc(39, 7);
            (0x00, 0x2c): operation_insertAndSend();
            (0x01, 0x2c): operation_insertAndSend();
            (0x02, 0x2c): operation_insertAndSend();
            (0x03, 0x2c): operation_insertAndCalcAndSend(40, 8, 7);
            (0x00, 0x2d): operation_insertAndSend();
            (0x01, 0x2d): operation_insertAndSend();
            (0x02, 0x2d): operation_insertAndSend();
            (0x03, 0x2d): operation_insertAndCalc(41, 8);
            (0x00, 0x2e): operation_insertAndSend();
            (0x01, 0x2e): operation_insertAndSend();
            (0x02, 0x2e): operation_insertAndSend();
            (0x03, 0x2e): operation_insertAndCalc(42, 8);
            (0x00, 0x2f): operation_insertAndSend();
            (0x01, 0x2f): operation_insertAndSend();
            (0x02, 0x2f): operation_insertAndSend();
            (0x03, 0x2f): operation_insertAndCalc(43, 8);
            (0x00, 0x30): operation_insertAndSend();
            (0x01, 0x30): operation_insertAndSend();
            (0x02, 0x30): operation_insertAndSend();
            (0x03, 0x30): operation_insertAndCalc(44, 8);
            (0x00, 0x31): operation_insertAndSend();
            (0x01, 0x31): operation_insertAndSend();
            (0x02, 0x31): operation_insertAndSend();
            (0x03, 0x31): operation_insertAndCalcAndSend(45, 9, 8);
            (0x00, 0x32): operation_insertAndSend();
            (0x01, 0x32): operation_insertAndSend();
            (0x02, 0x32): operation_insertAndSend();
            (0x03, 0x32): operation_insertAndCalc(46, 9);
            (0x00, 0x33): operation_insertAndSend();
            (0x01, 0x33): operation_insertAndSend();
            (0x02, 0x33): operation_insertAndSend();
            (0x03, 0x33): operation_insertAndCalc(47, 9);
            (0x00, 0x34): operation_insertAndSend();
            (0x01, 0x34): operation_insertAndSend();
            (0x02, 0x34): operation_insertAndSend();
            (0x03, 0x34): operation_insertAndCalc(48, 9);
            (0x00, 0x35): operation_insertAndSend();
            (0x01, 0x35): operation_insertAndSend();
            (0x02, 0x35): operation_insertAndSend();
            (0x03, 0x35): operation_insertAndCalc(49, 9);
            (0x00, 0x36): operation_insertAndSend();
            (0x01, 0x36): operation_insertAndSend();
            (0x02, 0x36): operation_insertAndSend();
            (0x03, 0x36): operation_insertAndCalcAndSend(50, 10, 9);
            (0x00, 0x37): operation_insertAndSend();
            (0x01, 0x37): operation_insertAndSend();
            (0x02, 0x37): operation_insertAndSend();
            (0x03, 0x37): operation_insertAndCalc(51, 10);
            (0x00, 0x38): operation_insertAndSend();
            (0x01, 0x38): operation_insertAndSend();
            (0x02, 0x38): operation_insertAndSend();
            (0x03, 0x38): operation_insertAndCalc(52, 10);
            (0x00, 0x39): operation_insertAndSend();
            (0x01, 0x39): operation_insertAndSend();
            (0x02, 0x39): operation_insertAndSend();
            (0x03, 0x39): operation_insertAndCalc(53, 10);
            (0x00, 0x3a): operation_insertAndSend();
            (0x01, 0x3a): operation_insertAndSend();
            (0x02, 0x3a): operation_insertAndSend();
            (0x03, 0x3a): operation_insertAndCalc(54, 10);
            (0x00, 0x3b): operation_insertAndSend();
            (0x01, 0x3b): operation_insertAndSend();
            (0x02, 0x3b): operation_insertAndSend();
            (0x03, 0x3b): operation_insertAndCalcAndSend(55, 11, 10);
            (0x00, 0x3c): operation_insertAndSend();
            (0x01, 0x3c): operation_insertAndSend();
            (0x02, 0x3c): operation_insertAndSend();
            (0x03, 0x3c): operation_insertAndCalc(56, 11);
            (0x00, 0x3d): operation_insertAndSend();
            (0x01, 0x3d): operation_insertAndSend();
            (0x02, 0x3d): operation_insertAndSend();
            (0x03, 0x3d): operation_insertAndCalc(57, 11);
            (0x00, 0x3e): operation_insertAndSend();
            (0x01, 0x3e): operation_insertAndSend();
            (0x02, 0x3e): operation_insertAndSend();
            (0x03, 0x3e): operation_insertAndCalc(58, 11);
            (0x00, 0x3f): operation_insertAndSend();
            (0x01, 0x3f): operation_insertAndSend();
            (0x02, 0x3f): operation_insertAndSend();
            (0x03, 0x3f): operation_insertAndCalc(59, 11);
            (0x00, 0x40): operation_insertAndSend();
            (0x01, 0x40): operation_insertAndSend();
            (0x02, 0x40): operation_insertAndSend();
            (0x03, 0x40): operation_insertAndCalcAndSend(60, 12, 11);
            (0x00, 0x41): operation_insertAndSend();
            (0x01, 0x41): operation_insertAndSend();
            (0x02, 0x41): operation_insertAndSend();
            (0x03, 0x41): operation_insertAndCalc(61, 12);
            (0x00, 0x42): operation_insertAndSend();
            (0x01, 0x42): operation_insertAndSend();
            (0x02, 0x42): operation_insertAndSend();
            (0x03, 0x42): operation_insertAndCalc(62, 12);
            (0x00, 0x43): operation_insertAndSend();
            (0x01, 0x43): operation_insertAndSend();
            (0x02, 0x43): operation_insertAndSend();
            (0x03, 0x43): operation_insertAndCalc(63, 12);
            (0x00, 0x44): operation_insertAndSend();
            (0x01, 0x44): operation_insertAndSend();
            (0x02, 0x44): operation_insertAndSend();
            (0x03, 0x44): operation_insertAndCalc(64, 12);
            (0x00, 0x45): operation_insertAndSend();
            (0x01, 0x45): operation_insertAndSend();
            (0x02, 0x45): operation_insertAndSend();
            (0x03, 0x45): operation_insertAndCalcAndSend(65, 13, 12);
            (0x00, 0x46): operation_insertAndSend();
            (0x01, 0x46): operation_insertAndSend();
            (0x02, 0x46): operation_insertAndSend();
            (0x03, 0x46): operation_insertAndCalc(66, 13);
            (0x00, 0x47): operation_insertAndSend();
            (0x01, 0x47): operation_insertAndSend();
            (0x02, 0x47): operation_insertAndSend();
            (0x03, 0x47): operation_insertAndCalc(67, 13);
            (0x00, 0x48): operation_insertAndSend();
            (0x01, 0x48): operation_insertAndSend();
            (0x02, 0x48): operation_insertAndSend();
            (0x03, 0x48): operation_insertAndCalc(68, 13);
            (0x00, 0x49): operation_insertAndSend();
            (0x01, 0x49): operation_insertAndSend();
            (0x02, 0x49): operation_insertAndSend();
            (0x03, 0x49): operation_insertAndCalc(69, 13);
            (0x00, 0x4a): operation_insertAndSend();
            (0x01, 0x4a): operation_insertAndSend();
            (0x02, 0x4a): operation_insertAndSend();
            (0x03, 0x4a): operation_insertAndCalcAndSend(70, 14, 13);
            (0x00, 0x4b): operation_insertAndSend();
            (0x01, 0x4b): operation_insertAndSend();
            (0x02, 0x4b): operation_insertAndSend();
            (0x03, 0x4b): operation_insertAndCalc(71, 14);
            (0x00, 0x4c): operation_insertAndSend();
            (0x01, 0x4c): operation_insertAndSend();
            (0x02, 0x4c): operation_insertAndSend();
            (0x03, 0x4c): operation_insertAndCalc(72, 14);
            (0x00, 0x4d): operation_insertAndSend();
            (0x01, 0x4d): operation_insertAndSend();
            (0x02, 0x4d): operation_insertAndSend();
            (0x03, 0x4d): operation_insertAndCalc(73, 14);
            (0x00, 0x4e): operation_insertAndSend();
            (0x01, 0x4e): operation_insertAndSend();
            (0x02, 0x4e): operation_insertAndSend();
            (0x03, 0x4e): operation_insertAndCalc(74, 14);
            (0x00, 0x4f): operation_insertAndSend();
            (0x01, 0x4f): operation_insertAndSend();
            (0x02, 0x4f): operation_insertAndSend();
            (0x03, 0x4f): operation_insertAndCalcAndSend(75, 15, 14);
            (0x00, 0x50): operation_insertAndSend();
            (0x01, 0x50): operation_insertAndSend();
            (0x02, 0x50): operation_insertAndSend();
            (0x03, 0x50): operation_insertAndCalc(76, 15);
            (0x00, 0x51): operation_insertAndSend();
            (0x01, 0x51): operation_insertAndSend();
            (0x02, 0x51): operation_insertAndSend();
            (0x03, 0x51): operation_insertAndCalc(77, 15);
            (0x00, 0x52): operation_insertAndSend();
            (0x01, 0x52): operation_insertAndSend();
            (0x02, 0x52): operation_insertAndSend();
            (0x03, 0x52): operation_insertAndCalc(78, 15);
            (0x00, 0x53): operation_insertAndSend();
            (0x01, 0x53): operation_insertAndSend();
            (0x02, 0x53): operation_insertAndSend();
            (0x03, 0x53): operation_insertAndCalc(79, 15);
            (0x00, 0x54): operation_insertAndSend();
            (0x01, 0x54): operation_insertAndSend();
            (0x02, 0x54): operation_insertAndSend();
            (0x03, 0x54): operation_insertAndCalcAndSend(80, 16, 15);
            (0x00, 0x55): operation_insertAndSend();
            (0x01, 0x55): operation_insertAndSend();
            (0x02, 0x55): operation_insertAndSend();
            (0x03, 0x55): operation_insertAndCalc(81, 16);
            (0x00, 0x56): operation_insertAndSend();
            (0x01, 0x56): operation_insertAndSend();
            (0x02, 0x56): operation_insertAndSend();
            (0x03, 0x56): operation_insertAndCalc(82, 16);
            (0x00, 0x57): operation_insertAndSend();
            (0x01, 0x57): operation_insertAndSend();
            (0x02, 0x57): operation_insertAndSend();
            (0x03, 0x57): operation_insertAndCalc(83, 16);
            (0x00, 0x58): operation_insertAndSend();
            (0x01, 0x58): operation_insertAndSend();
            (0x02, 0x58): operation_insertAndSend();
            (0x03, 0x58): operation_insertAndCalc(84, 16);
            (0x00, 0x59): operation_insertAndSend();
            (0x01, 0x59): operation_insertAndSend();
            (0x02, 0x59): operation_insertAndSend();
            (0x03, 0x59): operation_insertAndCalcAndSend(85, 17, 16);
            (0x00, 0x5a): operation_insertAndSend();
            (0x01, 0x5a): operation_insertAndSend();
            (0x02, 0x5a): operation_insertAndSend();
            (0x03, 0x5a): operation_insertAndCalc(86, 17);
            (0x00, 0x5b): operation_insertAndSend();
            (0x01, 0x5b): operation_insertAndSend();
            (0x02, 0x5b): operation_insertAndSend();
            (0x03, 0x5b): operation_insertAndCalc(87, 17);
            (0x00, 0x5c): operation_insertAndSend();
            (0x01, 0x5c): operation_insertAndSend();
            (0x02, 0x5c): operation_insertAndSend();
            (0x03, 0x5c): operation_insertAndCalc(88, 17);
            (0x00, 0x5d): operation_insertAndSend();
            (0x01, 0x5d): operation_insertAndSend();
            (0x02, 0x5d): operation_insertAndSend();
            (0x03, 0x5d): operation_insertAndCalc(89, 17);
            (0x00, 0x5e): operation_insertAndSend();
            (0x01, 0x5e): operation_insertAndSend();
            (0x02, 0x5e): operation_insertAndSend();
            (0x03, 0x5e): operation_insertAndCalcAndSend(90, 18, 17);
            (0x00, 0x5f): operation_insertAndSend();
            (0x01, 0x5f): operation_insertAndSend();
            (0x02, 0x5f): operation_insertAndSend();
            (0x03, 0x5f): operation_insertAndCalc(91, 18);
            (0x00, 0x60): operation_insertAndSend();
            (0x01, 0x60): operation_insertAndSend();
            (0x02, 0x60): operation_insertAndSend();
            (0x03, 0x60): operation_insertAndCalc(92, 18);
            (0x00, 0x61): operation_insertAndSend();
            (0x01, 0x61): operation_insertAndSend();
            (0x02, 0x61): operation_insertAndSend();
            (0x03, 0x61): operation_insertAndCalc(93, 18);
            (0x00, 0x62): operation_insertAndSend();
            (0x01, 0x62): operation_insertAndSend();
            (0x02, 0x62): operation_insertAndSend();
            (0x03, 0x62): operation_insertAndCalc(94, 18);
            (0x00, 0x63): operation_insertAndSend();
            (0x01, 0x63): operation_insertAndSend();
            (0x02, 0x63): operation_insertAndSend();
            (0x03, 0x63): operation_insertAndCalcAndSend(95, 19, 18);
            (0x00, 0x64): operation_insertAndSend();
            (0x01, 0x64): operation_insertAndSend();
            (0x02, 0x64): operation_insertAndSend();
            (0x03, 0x64): operation_insertAndCalc(96, 19);
            (0x00, 0x65): operation_insertAndSend();
            (0x01, 0x65): operation_insertAndSend();
            (0x02, 0x65): operation_insertAndSend();
            (0x03, 0x65): operation_insertAndCalc(97, 19);
            (0x00, 0x66): operation_insertAndSend();
            (0x01, 0x66): operation_insertAndSend();
            (0x02, 0x66): operation_insertAndSend();
            (0x03, 0x66): operation_insertAndCalc(98, 19);
            (0x00, 0x67): operation_insertAndSend();
            (0x01, 0x67): operation_insertAndSend();
            (0x02, 0x67): operation_insertAndSend();
            (0x03, 0x67): operation_insertAndCalc(99, 19);
            (0x00, 0x68): operation_insertAndSend();
            (0x01, 0x68): operation_insertAndSend();
            (0x02, 0x68): operation_insertAndSend();
            (0x03, 0x68): operation_insertAndCalcAndSend(100, 20, 19);
            (0x00, 0x69): operation_insertAndSend();
            (0x01, 0x69): operation_insertAndSend();
            (0x02, 0x69): operation_insertAndSend();
            (0x03, 0x69): operation_insertAndCalc(101, 20);
            (0x00, 0x6a): operation_insertAndSend();
            (0x01, 0x6a): operation_insertAndSend();
            (0x02, 0x6a): operation_insertAndSend();
            (0x03, 0x6a): operation_insertAndCalc(102, 20);
            (0x00, 0x6b): operation_insertAndSend();
            (0x01, 0x6b): operation_insertAndSend();
            (0x02, 0x6b): operation_insertAndSend();
            (0x03, 0x6b): operation_insertAndCalc(103, 20);
            (0x00, 0x6c): operation_insertAndSend();
            (0x01, 0x6c): operation_insertAndSend();
            (0x02, 0x6c): operation_insertAndSend();
            (0x03, 0x6c): operation_insertAndCalc(104, 20);
            (0x00, 0x6d): operation_insertAndSend();
            (0x01, 0x6d): operation_insertAndSend();
            (0x02, 0x6d): operation_insertAndSend();
            (0x03, 0x6d): operation_insertAndCalcAndSend(105, 21, 20);
            (0x00, 0x6e): operation_insertAndSend();
            (0x01, 0x6e): operation_insertAndSend();
            (0x02, 0x6e): operation_insertAndSend();
            (0x03, 0x6e): operation_insertAndCalc(106, 21);
            (0x00, 0x6f): operation_insertAndSend();
            (0x01, 0x6f): operation_insertAndSend();
            (0x02, 0x6f): operation_insertAndSend();
            (0x03, 0x6f): operation_insertAndCalc(107, 21);
            (0x00, 0x70): operation_insertAndSend();
            (0x01, 0x70): operation_insertAndSend();
            (0x02, 0x70): operation_insertAndSend();
            (0x03, 0x70): operation_insertAndCalc(108, 21);
            (0x00, 0x71): operation_insertAndSend();
            (0x01, 0x71): operation_insertAndSend();
            (0x02, 0x71): operation_insertAndSend();
            (0x03, 0x71): operation_insertAndCalc(109, 21);
            (0x00, 0x72): operation_insertAndSend();
            (0x01, 0x72): operation_insertAndSend();
            (0x02, 0x72): operation_insertAndSend();
            (0x03, 0x72): operation_insertAndCalcAndSend(110, 22, 21);
            (0x00, 0x73): operation_insertAndSend();
            (0x01, 0x73): operation_insertAndSend();
            (0x02, 0x73): operation_insertAndSend();
            (0x03, 0x73): operation_insertAndCalc(111, 22);
            (0x00, 0x74): operation_insertAndSend();
            (0x01, 0x74): operation_insertAndSend();
            (0x02, 0x74): operation_insertAndSend();
            (0x03, 0x74): operation_insertAndCalc(112, 22);
            (0x00, 0x75): operation_insertAndSend();
            (0x01, 0x75): operation_insertAndSend();
            (0x02, 0x75): operation_insertAndSend();
            (0x03, 0x75): operation_insertAndCalc(113, 22);
            (0x00, 0x76): operation_insertAndSend();
            (0x01, 0x76): operation_insertAndSend();
            (0x02, 0x76): operation_insertAndSend();
            (0x03, 0x76): operation_insertAndCalc(114, 22);
            (0x00, 0x77): operation_insertAndSend();
            (0x01, 0x77): operation_insertAndSend();
            (0x02, 0x77): operation_insertAndSend();
            (0x03, 0x77): operation_insertAndCalcAndSend(115, 23, 22);
            (0x00, 0x78): operation_insertAndSend();
            (0x01, 0x78): operation_insertAndSend();
            (0x02, 0x78): operation_insertAndSend();
            (0x03, 0x78): operation_insertAndCalc(116, 23);
            (0x00, 0x79): operation_insertAndSend();
            (0x01, 0x79): operation_insertAndSend();
            (0x02, 0x79): operation_insertAndSend();
            (0x03, 0x79): operation_insertAndCalc(117, 23);
            (0x00, 0x7a): operation_insertAndSend();
            (0x01, 0x7a): operation_insertAndSend();
            (0x02, 0x7a): operation_insertAndSend();
            (0x03, 0x7a): operation_insertAndCalc(118, 23);
            (0x00, 0x7b): operation_insertAndSend();
            (0x01, 0x7b): operation_insertAndSend();
            (0x02, 0x7b): operation_insertAndSend();
            (0x03, 0x7b): operation_insertAndCalc(119, 23);
            (0x00, 0x7c): operation_insertAndSend();
            (0x01, 0x7c): operation_insertAndSend();
            (0x02, 0x7c): operation_insertAndSend();
            (0x03, 0x7c): operation_insertAndCalcAndSend(120, 24, 23);
            (0x00, 0x7d): operation_insertAndSend();
            (0x01, 0x7d): operation_insertAndSend();
            (0x02, 0x7d): operation_insertAndSend();
            (0x03, 0x7d): operation_insertAndCalc(121, 24);
            (0x00, 0x7e): operation_insertAndSend();
            (0x01, 0x7e): operation_insertAndSend();
            (0x02, 0x7e): operation_insertAndSend();
            (0x03, 0x7e): operation_insertAndCalc(122, 24);
            (0x00, 0x7f): operation_insertAndSend();
            (0x01, 0x7f): operation_insertAndSend();
            (0x02, 0x7f): operation_insertAndSend();
            (0x03, 0x7f): operation_insertAndCalc(123, 24);
            (0x00, 0x80): operation_insertAndSend();
            (0x01, 0x80): operation_insertAndSend();
            (0x02, 0x80): operation_insertAndSend();
            (0x03, 0x80): operation_insertAndCalc(124, 24);
            (0x00, 0x81): operation_insertAndSend();
            (0x01, 0x81): operation_insertAndSend();
            (0x02, 0x81): operation_insertAndSend();
            (0x03, 0x81): operation_insertAndCalcAndSend(125, 25, 24);
            (0x00, 0x82): operation_insertAndSend();
            (0x01, 0x82): operation_insertAndSend();
            (0x02, 0x82): operation_insertAndSend();
            (0x03, 0x82): operation_insertAndCalc(126, 25);
            (0x00, 0x83): operation_insertAndSend();
            (0x01, 0x83): operation_insertAndSend();
            (0x02, 0x83): operation_insertAndSend();
            (0x03, 0x83): operation_insertAndCalc(127, 25);
            (0x00, 0x84): operation_insertAndSend();
            (0x01, 0x84): operation_insertAndSend();
            (0x02, 0x84): operation_insertAndSend();
            (0x03, 0x84): operation_insertAndCalc(128, 25);
            (0x00, 0x85): operation_insertAndSend();
            (0x01, 0x85): operation_insertAndSend();
            (0x02, 0x85): operation_insertAndSend();
            (0x03, 0x85): operation_insertAndCalc(129, 25);
            (0x00, 0x86): operation_insertAndSend();
            (0x01, 0x86): operation_insertAndSend();
            (0x02, 0x86): operation_insertAndSend();
            (0x03, 0x86): operation_insertAndCalcAndSend(130, 26, 25);
            (0x00, 0x87): operation_insertAndSend();
            (0x01, 0x87): operation_insertAndSend();
            (0x02, 0x87): operation_insertAndSend();
            (0x03, 0x87): operation_insertAndCalc(131, 26);
            (0x00, 0x88): operation_insertAndSend();
            (0x01, 0x88): operation_insertAndSend();
            (0x02, 0x88): operation_insertAndSend();
            (0x03, 0x88): operation_insertAndCalc(132, 26);
            (0x00, 0x89): operation_insertAndSend();
            (0x01, 0x89): operation_insertAndSend();
            (0x02, 0x89): operation_insertAndSend();
            (0x03, 0x89): operation_insertAndCalc(133, 26);
            (0x00, 0x8a): operation_insertAndSend();
            (0x01, 0x8a): operation_insertAndSend();
            (0x02, 0x8a): operation_insertAndSend();
            (0x03, 0x8a): operation_insertAndCalc(134, 26);
            (0x00, 0x8b): operation_insertAndSend();
            (0x01, 0x8b): operation_insertAndSend();
            (0x02, 0x8b): operation_insertAndSend();
            (0x03, 0x8b): operation_insertAndCalcAndSend(135, 27, 26);
            (0x00, 0x8c): operation_insertAndSend();
            (0x01, 0x8c): operation_insertAndSend();
            (0x02, 0x8c): operation_insertAndSend();
            (0x03, 0x8c): operation_insertAndCalc(136, 27);
            (0x00, 0x8d): operation_insertAndSend();
            (0x01, 0x8d): operation_insertAndSend();
            (0x02, 0x8d): operation_insertAndSend();
            (0x03, 0x8d): operation_insertAndCalc(137, 27);
            (0x00, 0x8e): operation_insertAndSend();
            (0x01, 0x8e): operation_insertAndSend();
            (0x02, 0x8e): operation_insertAndSend();
            (0x03, 0x8e): operation_insertAndCalc(138, 27);
            (0x00, 0x8f): operation_insertAndSend();
            (0x01, 0x8f): operation_insertAndSend();
            (0x02, 0x8f): operation_insertAndSend();
            (0x03, 0x8f): operation_insertAndCalc(139, 27);
            (0x00, 0x90): operation_insertAndSend();
            (0x01, 0x90): operation_insertAndSend();
            (0x02, 0x90): operation_insertAndSend();
            (0x03, 0x90): operation_insertAndCalcAndSend(140, 28, 27);
            (0x00, 0x91): operation_insertAndSend();
            (0x01, 0x91): operation_insertAndSend();
            (0x02, 0x91): operation_insertAndSend();
            (0x03, 0x91): operation_insertAndCalc(141, 28);
            (0x00, 0x92): operation_insertAndSend();
            (0x01, 0x92): operation_insertAndSend();
            (0x02, 0x92): operation_insertAndSend();
            (0x03, 0x92): operation_insertAndCalc(142, 28);
            (0x00, 0x93): operation_insertAndSend();
            (0x01, 0x93): operation_insertAndSend();
            (0x02, 0x93): operation_insertAndSend();
            (0x03, 0x93): operation_insertAndCalc(143, 28);
            (0x00, 0x94): operation_insertAndSend();
            (0x01, 0x94): operation_insertAndSend();
            (0x02, 0x94): operation_insertAndSend();
            (0x03, 0x94): operation_insertAndCalc(144, 28);
            (0x00, 0x95): operation_insertAndSend();
            (0x01, 0x95): operation_insertAndSend();
            (0x02, 0x95): operation_insertAndSend();
            (0x03, 0x95): operation_insertAndCalcAndSend(145, 29, 28);
            (0x00, 0x96): operation_insertAndSend();
            (0x01, 0x96): operation_insertAndSend();
            (0x02, 0x96): operation_insertAndSend();
            (0x03, 0x96): operation_insertAndCalc(146, 29);
            (0x00, 0x97): operation_insertAndSend();
            (0x01, 0x97): operation_insertAndSend();
            (0x02, 0x97): operation_insertAndSend();
            (0x03, 0x97): operation_insertAndCalc(147, 29);
            (0x00, 0x98): operation_insertAndSend();
            (0x01, 0x98): operation_insertAndSend();
            (0x02, 0x98): operation_insertAndSend();
            (0x03, 0x98): operation_insertAndCalc(148, 29);
            (0x00, 0x99): operation_insertAndSend();
            (0x01, 0x99): operation_insertAndSend();
            (0x02, 0x99): operation_insertAndSend();
            (0x03, 0x99): operation_insertAndCalc(149, 29);
            (0x00, 0x9a): operation_insertAndSend();
            (0x01, 0x9a): operation_insertAndSend();
            (0x02, 0x9a): operation_insertAndSend();
            (0x03, 0x9a): operation_insertAndCalcAndSend(150, 30, 29);
            (0x00, 0x9b): operation_insertAndSend();
            (0x01, 0x9b): operation_insertAndSend();
            (0x02, 0x9b): operation_insertAndSend();
            (0x03, 0x9b): operation_insertAndCalc(151, 30);
            (0x00, 0x9c): operation_insertAndSend();
            (0x01, 0x9c): operation_insertAndSend();
            (0x02, 0x9c): operation_insertAndSend();
            (0x03, 0x9c): operation_insertAndCalc(152, 30);
            (0x00, 0x9d): operation_insertAndSend();
            (0x01, 0x9d): operation_insertAndSend();
            (0x02, 0x9d): operation_insertAndSend();
            (0x03, 0x9d): operation_insertAndCalc(153, 30);
            (0x00, 0x9e): operation_insertAndSend();
            (0x01, 0x9e): operation_insertAndSend();
            (0x02, 0x9e): operation_insertAndSend();
            (0x03, 0x9e): operation_insertAndCalc(154, 30);
            (0x00, 0x9f): operation_insertAndSend();
            (0x01, 0x9f): operation_insertAndSend();
            (0x02, 0x9f): operation_insertAndSend();
